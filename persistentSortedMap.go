...
/*
 Implements persistent sorted maps -- based on the PersistentTreeMap.java from Clojure.
...
func (m *persistentSortedMap) ValueAt(key string) Value {
	n := m.entryAt(key)
	if n != nil {
		return n.val()
	}
...
func (m *persistentSortedMap) entryAt(key string) treeNode {
	t := m.tree
	for t != nil {
		if key == t.key() {
			return t
